//
//     Generated by class-dump 3.5 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard.
//

#import <objc/NSObject.h>

@class FinderClientStatus, FinderLivePermissionVerifyInfo, FinderLiveReportBaseInfo, FinderNativeDramaSessionInfo, FinderNativeSourceInfo, FinderShareUserInfo, JsApiSourceInfo, NSData, NSString, StreamTabTipsInfo, WCFinderGetDetailDataItemCGIConfig, WCFinderPushFeedViewProductParams;

@interface WCFinderGetCommentDetailDataItemCGIParams : NSObject
{
    _Bool _preloadFirstFrameInWIFI;
    _Bool _isPrefetchRequest;
    _Bool _ignoreAutoToast;
    _Bool _takeOverBusinessErrMsg;
    unsigned int _bypassType;
    NSString *_containerId;
    StreamTabTipsInfo *_tabTipsInfo;
    WCFinderPushFeedViewProductParams *_productInfo;
    NSString *_fluencyReportSceneKey;
    WCFinderGetDetailDataItemCGIConfig *_config;
    FinderLivePermissionVerifyInfo *_verifyInfo;
    FinderClientStatus *_clientStatus;
    FinderLiveReportBaseInfo *_liveReportBaseInfo;
    NSString *_gMsgId;
    JsApiSourceInfo *_jsApiSourceInfo;
    FinderNativeSourceInfo *_nativeInfo;
    unsigned long long _relatedScene;
    NSString *_fromSessionId;
    NSData *_bypassData;
    NSData *_likeBuffer;
    FinderShareUserInfo *_shareUserInfo;
    FinderNativeDramaSessionInfo *_nativeDramaSessionInfo;
}

- (void).cxx_destruct;
@property(retain, nonatomic) FinderNativeDramaSessionInfo *nativeDramaSessionInfo; // @synthesize nativeDramaSessionInfo=_nativeDramaSessionInfo;
@property(nonatomic) _Bool takeOverBusinessErrMsg; // @synthesize takeOverBusinessErrMsg=_takeOverBusinessErrMsg;
@property(nonatomic) _Bool ignoreAutoToast; // @synthesize ignoreAutoToast=_ignoreAutoToast;
@property(retain, nonatomic) FinderShareUserInfo *shareUserInfo; // @synthesize shareUserInfo=_shareUserInfo;
@property(retain, nonatomic) NSData *likeBuffer; // @synthesize likeBuffer=_likeBuffer;
@property(retain, nonatomic) NSData *bypassData; // @synthesize bypassData=_bypassData;
@property(nonatomic) unsigned int bypassType; // @synthesize bypassType=_bypassType;
@property(copy, nonatomic) NSString *fromSessionId; // @synthesize fromSessionId=_fromSessionId;
@property(nonatomic) unsigned long long relatedScene; // @synthesize relatedScene=_relatedScene;
@property(retain, nonatomic) FinderNativeSourceInfo *nativeInfo; // @synthesize nativeInfo=_nativeInfo;
@property(retain, nonatomic) JsApiSourceInfo *jsApiSourceInfo; // @synthesize jsApiSourceInfo=_jsApiSourceInfo;
@property(copy, nonatomic) NSString *gMsgId; // @synthesize gMsgId=_gMsgId;
@property(retain, nonatomic) FinderLiveReportBaseInfo *liveReportBaseInfo; // @synthesize liveReportBaseInfo=_liveReportBaseInfo;
@property(retain, nonatomic) FinderClientStatus *clientStatus; // @synthesize clientStatus=_clientStatus;
@property(retain, nonatomic) FinderLivePermissionVerifyInfo *verifyInfo; // @synthesize verifyInfo=_verifyInfo;
@property(retain, nonatomic) WCFinderGetDetailDataItemCGIConfig *config; // @synthesize config=_config;
@property(copy, nonatomic) NSString *fluencyReportSceneKey; // @synthesize fluencyReportSceneKey=_fluencyReportSceneKey;
@property(nonatomic) _Bool isPrefetchRequest; // @synthesize isPrefetchRequest=_isPrefetchRequest;
@property(nonatomic) _Bool preloadFirstFrameInWIFI; // @synthesize preloadFirstFrameInWIFI=_preloadFirstFrameInWIFI;
@property(retain, nonatomic) WCFinderPushFeedViewProductParams *productInfo; // @synthesize productInfo=_productInfo;
@property(retain, nonatomic) StreamTabTipsInfo *tabTipsInfo; // @synthesize tabTipsInfo=_tabTipsInfo;
@property(copy, nonatomic) NSString *containerId; // @synthesize containerId=_containerId;

@end

