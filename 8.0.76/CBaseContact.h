//
//     Generated by class-dump 3.5 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard.
//

#import <objc/NSObject.h>

@class NSData, NSDictionary, NSString;

@interface CBaseContact : NSObject
{
    NSString *m_nsUsrName;
    NSString *m_nsEncodeUserName;
    NSString *m_nsAliasName;
    unsigned int m_uiConType;
    NSString *m_nsNickName;
    NSString *m_nsFullPY;
    NSString *m_nsShortPY;
    NSString *m_nsRemark;
    NSString *m_nsRemarkPYShort;
    NSString *m_nsRemarkPYFull;
    NSString *m_nsDisplayNamePY;
    unsigned int m_uiSex;
    unsigned int m_uiType;
    unsigned long long m_uiTypeExt;
    unsigned int m_uiChatState;
    NSData *m_dtUsrImg;
    NSString *m_nsImgStatus;
    NSString *m_nsHDImgStatus;
    NSString *m_nsHeadImgUrl;
    NSString *m_nsHeadHDImgUrl;
    NSString *m_nsHeadHDMd5;
    NSString *m_nsDraft;
    unsigned int m_uiDraftTime;
    NSString *m_nsAtUserList;
    unsigned int m_uiQQUin;
    NSString *m_nsMobileIdentify;
    unsigned int m_uiFriendScene;
    unsigned int m_uiImgKey;
    unsigned int m_uiExtKey;
    unsigned int m_uiImgKeyAtLastGet;
    unsigned int m_uiExtKeyAtLastGet;
    _Bool m_hasDetectPlugin;
    _Bool m_isPlugin;
    _Bool m_hasDetectSelf;
    _Bool m_isSelf;
    NSString *m_nsAntispamTicket;
    _Bool _m_bUseHeadImgUrlForDisplay;
    _Bool _m_bUseRoundHeadImage;
    long long m_i64ReferMsgSvrId;
    NSString *m_nsFriendUserName;
    CBaseContact *_bypRoleContact;
    CBaseContact *_bypRealChatContact;
    NSDictionary *_externalInfoJSONCache;
    NSString *_m_nsGroupMemberTicket;
    NSString *_m_authIconUrl;
}

+ (_Bool)isWeChatPayContactUsername:(id)arg1;
+ (_Bool)isWeChatPayCollAssistUsername:(id)arg1;
+ (void)initialize;
+ (void)PBArrayAdd_m_uiTypeExt;
+ (void)PBArrayAdd_m_nsFriendUserName;
+ (void)PBArrayAdd_m_i64ReferMsgSvrId;
+ (void)PBArrayAdd_m_nsAtUserList;
+ (void)PBArrayAdd_m_uiDraftTime;
+ (void)PBArrayAdd_m_uiFriendScene;
+ (void)PBArrayAdd_m_nsMobileIdentify;
+ (void)PBArrayAdd_m_uiQQUin;
+ (void)PBArrayAdd_m_uiExtKeyAtLastGet;
+ (void)PBArrayAdd_m_uiImgKeyAtLastGet;
+ (void)PBArrayAdd_m_uiExtKey;
+ (void)PBArrayAdd_m_uiImgKey;
+ (void)PBArrayAdd_m_nsDraft;
+ (void)PBArrayAdd_m_nsHeadHDMd5;
+ (void)PBArrayAdd_m_nsHeadHDImgUrl;
+ (void)PBArrayAdd_m_nsHeadImgUrl;
+ (void)PBArrayAdd_m_nsHDImgStatus;
+ (void)PBArrayAdd_m_nsImgStatus;
+ (void)PBArrayAdd_m_uiChatState;
+ (void)PBArrayAdd_m_uiType;
+ (void)PBArrayAdd_m_uiSex;
+ (void)PBArrayAdd_m_nsRemarkPYFull;
+ (void)PBArrayAdd_m_nsRemarkPYShort;
+ (void)PBArrayAdd_m_nsRemark;
+ (void)PBArrayAdd_m_nsFullPY;
+ (void)PBArrayAdd_m_nsNickName;
+ (void)PBArrayAdd_m_uiConType;
+ (void)PBArrayAdd_m_nsAliasName;
+ (void)PBArrayAdd_m_nsEncodeUserName;
+ (void)PBArrayAdd_m_nsUsrName;
- (void).cxx_destruct;
@property(retain, nonatomic) NSString *m_authIconUrl; // @synthesize m_authIconUrl=_m_authIconUrl;
@property(retain, nonatomic) NSString *m_nsGroupMemberTicket; // @synthesize m_nsGroupMemberTicket=_m_nsGroupMemberTicket;
@property(retain) NSDictionary *externalInfoJSONCache; // @synthesize externalInfoJSONCache=_externalInfoJSONCache;
@property(retain, nonatomic) CBaseContact *bypRealChatContact; // @synthesize bypRealChatContact=_bypRealChatContact;
@property(retain, nonatomic) CBaseContact *bypRoleContact; // @synthesize bypRoleContact=_bypRoleContact;
@property(nonatomic) _Bool m_bUseRoundHeadImage; // @synthesize m_bUseRoundHeadImage=_m_bUseRoundHeadImage;
@property(nonatomic) _Bool m_bUseHeadImgUrlForDisplay; // @synthesize m_bUseHeadImgUrlForDisplay=_m_bUseHeadImgUrlForDisplay;
@property(retain) NSString *m_nsDisplayNamePY; // @synthesize m_nsDisplayNamePY;
@property(retain, nonatomic) NSString *m_nsAntispamTicket; // @synthesize m_nsAntispamTicket;
@property(retain, nonatomic) NSString *m_nsShortPY; // @synthesize m_nsShortPY;
@property(nonatomic) unsigned long long m_uiTypeExt; // @synthesize m_uiTypeExt;
@property(retain, nonatomic) NSString *m_nsFriendUserName; // @synthesize m_nsFriendUserName;
@property(nonatomic) long long m_i64ReferMsgSvrId; // @synthesize m_i64ReferMsgSvrId;
@property(retain, nonatomic) NSString *m_nsAtUserList; // @synthesize m_nsAtUserList;
@property(nonatomic) unsigned int m_uiDraftTime; // @synthesize m_uiDraftTime;
@property(nonatomic) unsigned int m_uiFriendScene; // @synthesize m_uiFriendScene;
@property(retain, nonatomic) NSString *m_nsMobileIdentify; // @synthesize m_nsMobileIdentify;
@property(nonatomic) unsigned int m_uiQQUin; // @synthesize m_uiQQUin;
@property(nonatomic) unsigned int m_uiExtKeyAtLastGet; // @synthesize m_uiExtKeyAtLastGet;
@property(nonatomic) unsigned int m_uiImgKeyAtLastGet; // @synthesize m_uiImgKeyAtLastGet;
@property(nonatomic) unsigned int m_uiExtKey; // @synthesize m_uiExtKey;
@property(nonatomic) unsigned int m_uiImgKey; // @synthesize m_uiImgKey;
@property(retain, nonatomic) NSString *m_nsDraft; // @synthesize m_nsDraft;
@property(retain, nonatomic) NSString *m_nsHeadHDMd5; // @synthesize m_nsHeadHDMd5;
@property(retain) NSString *m_nsHeadHDImgUrl; // @synthesize m_nsHeadHDImgUrl;
@property(retain, nonatomic) NSString *m_nsHeadImgUrl; // @synthesize m_nsHeadImgUrl;
@property(retain, nonatomic) NSString *m_nsHDImgStatus; // @synthesize m_nsHDImgStatus;
@property(retain, nonatomic) NSString *m_nsImgStatus; // @synthesize m_nsImgStatus;
@property(retain, nonatomic) NSData *m_dtUsrImg; // @synthesize m_dtUsrImg;
@property(nonatomic) unsigned int m_uiChatState; // @synthesize m_uiChatState;
@property(nonatomic) unsigned int m_uiType; // @synthesize m_uiType;
@property(nonatomic) unsigned int m_uiSex; // @synthesize m_uiSex;
@property(retain) NSString *m_nsRemarkPYFull; // @synthesize m_nsRemarkPYFull;
@property(retain, nonatomic) NSString *m_nsRemarkPYShort; // @synthesize m_nsRemarkPYShort;
@property(retain, nonatomic) NSString *m_nsRemark; // @synthesize m_nsRemark;
@property(retain) NSString *m_nsFullPY; // @synthesize m_nsFullPY;
@property(nonatomic) unsigned int m_uiConType; // @synthesize m_uiConType;
@property(retain, nonatomic) NSString *m_nsAliasName; // @synthesize m_nsAliasName;
@property(retain, nonatomic) NSString *m_nsEncodeUserName; // @synthesize m_nsEncodeUserName;
@property(retain, nonatomic) NSString *m_nsUsrName; // @synthesize m_nsUsrName;
@property(readonly, nonatomic) _Bool m_isPlugin; // @synthesize m_isPlugin;
- (int)getImageStatusCode;
- (id)getQQDisplayName;
- (_Bool)isHasQQ;
- (_Bool)isWeixinCustomerHelperContact;
- (_Bool)isWeixinTeamContact;
- (_Bool)isSelf;
- (_Bool)hasContactDisplayUsrName;
- (id)getContactDisplayUsrName;
- (id)getPluginPresentedContactDisplayName;
- (id)getContactDisplayName:(unsigned int *)arg1;
- (id)getContactDisplayName;
- (void)setDisplayNamePY;
- (id)getRemark;
- (void)saveUserImage;
- (id)getContactHeadImage;
- (_Bool)isNeedGetHDImg;
- (_Bool)isHasHDImg;
- (_Bool)isNeedGetUsrImgWithoutCheckLocalFile;
- (_Bool)isNeedGetUsrImg;
- (_Bool)isEnterpriseContact;
- (_Bool)isUseSvrHeadImagePluginContact;
- (_Bool)isPluginPresentedContact;
- (_Bool)isMerchantAssistContact;
- (_Bool)isWeChatPayHKContact;
- (_Bool)isMinishopAssistContact;
- (_Bool)isWeChatPayCollAssistContact;
- (_Bool)isWeChatPayContact;
- (_Bool)isWeGameContact;
- (_Bool)isWeSportContact;
- (_Bool)isChatStatusNotifyOpen;
- (_Bool)isWatchHolder;
- (_Bool)isMacHelper;
- (_Bool)isQQ;
- (_Bool)isQQMBlog;
- (_Bool)isFavWeAppMsgHolder;
- (_Bool)isTemplateMsgHolder;
- (_Bool)isFileHelper;
- (_Bool)isQAAssistant;
- (_Bool)enableChatBotVoIP;
- (_Bool)isPersonalAssistant;
- (_Bool)isGuangYao;
- (_Bool)enableChatBotVoIPAndShouldMultiTalkMode;
- (_Bool)isAiVoIPDemo;
- (_Bool)isAIAssistant;
- (_Bool)isCustAsst;
- (_Bool)isWeClawBot;
- (_Bool)isChatBot;
- (_Bool)isWWKFSessionHolder;
- (_Bool)isBrandSessionHolder;
- (_Bool)isGroupCard;
- (_Bool)isOpenImKefuContact;
- (_Bool)isOpenImContact;
- (_Bool)isOpenIMChatroom;
- (_Bool)isChatroom;
- (_Bool)isLbsroom;
- (_Bool)isWeixin;
- (_Bool)isChatRoomMember;
- (_Bool)isInChatList;
- (_Bool)isMMContact;
- (_Bool)isFavour;
- (void)setWeSportBlack:(_Bool)arg1;
- (_Bool)isWeSportBlack;
- (_Bool)isFinderUser;
- (_Bool)isFinderBlack;
- (_Bool)isFinderNointeresting;
- (void)setMyTeenagerModeGuardian:(_Bool)arg1;
- (_Bool)isMyTeenagerModeGuardian;
- (void)setMyTeenagerModeWard:(_Bool)arg1;
- (_Bool)isMyTeenagerModeWard;
- (void)setSnsBlack:(_Bool)arg1;
- (_Bool)isSnsBlack;
- (void)setSocialBlack:(_Bool)arg1;
- (_Bool)isSocialBlack;
- (void)setBlack:(_Bool)arg1;
- (_Bool)isBlack;
- (_Bool)isEqualToName:(id)arg1;
- (_Bool)isEqualToContact:(id)arg1;
- (id)getEncodeUserName;
- (_Bool)isValid;
- (void)setChatRoomTopic:(id)arg1;
- (id)chatRoomTopic;
- (long long)compare:(id)arg1;
@property(readonly, copy) NSString *description;
- (_Bool)copyFrom:(id)arg1;
- (id)initWithCoder:(id)arg1;
- (void)encodeWithCoder:(id)arg1;
- (id)init;
- (void)setRemarkWithoutEmojiChange:(id)arg1;
- (void)setNickNameWithoutEmojiChange:(id)arg1;
- (void)tryHeadImgUrl:(id)arg1;
- (void)tryNickName:(id)arg1;
@property(retain) NSString *m_nsNickName; // @synthesize m_nsNickName;
- (unsigned int)realFriendScene;
- (id)getPBPropertyTable;
@property(nonatomic) unsigned int forwardMessageContactFromScene;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
// Preceding property had unknown attributes: ?
// Original attribute string: T@"NSString",?,R,C

@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

