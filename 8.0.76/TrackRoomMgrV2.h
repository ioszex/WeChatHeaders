//
//     Generated by class-dump 3.5 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard.
//

@class MMTimer, NSArray, NSMutableDictionary, NSString, NSTimer, TrackPOIItem, TrackRoomReportParam;

@interface TrackRoomMgrV2
{
    _Bool _isJoining;
    _Bool _isRefreshing;
    int _nextState;
    int _currentState;
    unsigned long long _headingTag;
    unsigned long long _locationTag;
    int _retryCount;
    double _trueHeading;
    double _course;
    NSString *_chatname;
    NSString *_trackRoomID;
    NSString *_uniqueTrackRoomID;
    long long _joinTimestamp;
    long long _onlineUserCount;
    TrackPOIItem *_trackPOIItem;
    TrackPOIItem *_roomPOI;
    struct CLLocationCoordinate2D _uploadCoordinate;
    MMTimer *_refreshTimer;
    unsigned int _scene;
    unsigned int _refreshTimeInterval;
    TrackRoomReportParam *_exitReportParam;
    NSArray *_latestOtherPositions;
    _Bool _hasLegacyUser;
    _Bool _isMinimized;
    NSTimer *_leaveTimeoutTimer;
    _Bool _isNextResumeFromFloat;
    _Bool _hasExitCurrentChat;
    long long _pendingCloseMethod;
    long long _snapshotCloseMethodForReport;
    _Bool _snapshotHasExitCurrentChatForReport;
    _Bool _wasMinimizedAtCloseTime;
    NSMutableDictionary *_chatEnterTimestamps;
    _Bool _hasReportedFirstRefresh;
    struct CLLocationCoordinate2D _coordinate;
}

+ (id)tipsTextForUserList:(id)arg1;
- (void).cxx_destruct;
@property(readonly, nonatomic) _Bool hasLegacyUser; // @synthesize hasLegacyUser=_hasLegacyUser;
@property(readonly, nonatomic) TrackPOIItem *roomPOI; // @synthesize roomPOI=_roomPOI;
@property(readonly, nonatomic) unsigned int refreshTimeInterval; // @synthesize refreshTimeInterval=_refreshTimeInterval;
@property(readonly, nonatomic) struct CLLocationCoordinate2D coordinate; // @synthesize coordinate=_coordinate;
@property(readonly, nonatomic) NSString *trackRoomID; // @synthesize trackRoomID=_trackRoomID;
@property(readonly, nonatomic) NSString *chatname; // @synthesize chatname=_chatname;
- (void)setPendingCloseMethod:(long long)arg1;
- (id)eventContext;
- (long long)chatEnterTimestampOf:(id)arg1;
- (id)createTrackRoomReportParamType:(long long)arg1 code:(long long)arg2 chatname:(id)arg3 roomId:(id)arg4 uniqueRoomId:(id)arg5 joinTimestamp:(long long)arg6 onlineUserCount:(long long)arg7 isRoomCreate:(_Bool)arg8 isRoomDestroy:(_Bool)arg9;
- (long long)currentMinDistanceToOthers;
- (void)reportParam:(id)arg1;
- (void)reportType:(long long)arg1 code:(long long)arg2 chatname:(id)arg3 roomId:(id)arg4 uniqueRoomId:(id)arg5 joinTimestamp:(long long)arg6 onlineUserCount:(long long)arg7 isRoomCreate:(_Bool)arg8 isRoomDestroy:(_Bool)arg9;
- (void)reportScene:(long long)arg1 code:(long long)arg2 isRecover:(_Bool)arg3 chatname:(id)arg4 roomId:(id)arg5 uniqueRoomId:(id)arg6 joinTimestamp:(long long)arg7 onlineUserCount:(long long)arg8 isRoomCreate:(_Bool)arg9 isRoomDestroy:(_Bool)arg10;
- (void)onGPSLocationChanged:(id)arg1 withTag:(unsigned long long)arg2;
- (void)onHeadingError:(int)arg1 withTag:(unsigned long long)arg2;
- (void)onHeadingChanged:(id)arg1 withTag:(unsigned long long)arg2;
- (void)MessageReturn:(id)arg1 Event:(unsigned int)arg2;
- (void)handleExitTrackRoomResponse:(id)arg1;
- (void)handleRefreshTrackRoomResponse:(id)arg1;
- (void)handleJoinTrackRoomResponse:(id)arg1;
- (void)retryChangeState;
- (int)exitTrackRoom:(unsigned int)arg1;
- (void)onTimerRefreshTrackRoom;
- (int)refreshTrackRoomWithType:(int)arg1;
- (int)joinTrackRoom:(id)arg1;
- (void)stopUpdateCourse;
- (void)startUpdateCourse;
- (void)stopUpdateTrueHeading;
- (void)startUpdateTrueHeading;
- (void)setCurrentState:(int)arg1;
- (int)onRoomStateChanging:(int)arg1;
@property(readonly, nonatomic) _Bool isUploading;
@property(readonly, nonatomic) double heading;
- (int)closeTrackRoom:(unsigned int)arg1;
- (void)setUploadCoordinate:(struct CLLocationCoordinate2D)arg1;
- (int)setTrackRoomState:(int)arg1;
- (void)collapseTrackRoom;
- (void)markNextResumeFromFloat;
- (void)resumeTrackRoom:(unsigned int)arg1;
- (void)onLeaveTimeoutFired;
- (void)stopLeaveTimeoutTimer;
- (void)startLeaveTimeoutTimer;
- (void)onMaximize;
- (void)onMinimize;
- (void)onSessionDisappear:(id)arg1;
- (void)onSessionAppear:(id)arg1;
- (int)openTrackRoom:(id)arg1 POIItem:(id)arg2 Scene:(unsigned int)arg3;
- (void)enterForeground;
- (void)enterBackground;
- (void)dealloc;
- (id)init;
- (void)reportExitAction:(long long)arg1;
- (void)reportTimeAction:(long long)arg1 time:(unsigned long long)arg2;
- (void)reportRetryAction:(long long)arg1;
- (void)reportAction:(long long)arg1;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
// Preceding property had unknown attributes: ?
// Original attribute string: T@"NSString",?,R,C

@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

