//
//     Generated by class-dump 3.5 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard.
//

#import <objc/NSObject.h>

@class CdnRenderViewBase, CustomPlayerConfig, FinderLiveVoipStreamStatus, MMLiveAudioVolumeManageLogic, MMLiveCDNPlayerCustomExtraPram, MMLiveCDNPlayerStreamWrap, MMLiveCdnStreamInfo, MMLiveCommonSEIMsg, MMLiveTask, NSArray, NSMutableDictionary, NSString, TRTCCdnPlayerManager, UIView;
@protocol MMLiveCDNPlayerItemDelegate;

@interface MMLiveCDNPlayerItem : NSObject
{
    _Bool _hasPlayBegan;
    _Bool _hasReceivedFirstIFrame;
    _Bool _hasConnectedSuccess;
    _Bool _isPlayingLocalAudioLiveStream;
    _Bool _interruptedWhenAppResignActive;
    _Bool _shouldStartInBackground;
    _Bool _isPreplayMode;
    _Bool _isMinimizeMode;
    _Bool _isAudioMode;
    _Bool _isAudioRoomMode;
    _Bool _isKTVMode;
    _Bool _isGameMode;
    _Bool _isToBeMuted;
    _Bool _enableBackgroundDecoding;
    _Bool _needCacheRecentCdnPixelBuffer;
    _Bool _serverSwitchRciRendering;
    _Bool _hasSetPlayerOrientation;
    _Bool _hasSetPlayerRenderMode;
    _Bool _hasSetPlayerCacheType;
    _Bool _hasSetPlayerCacheConfig;
    _Bool _hasEnteredBackgroundWhenWaitingPlayingOK;
    _Bool _enableBackgroundRendering;
    int _liveVrType;
    int _encryptMode;
    int _channelCount;
    int _sampleRate;
    int _commentScene;
    int _minPlayerCache;
    int _maxPlayerCache;
    int _minPlayerCacheLeb;
    int _maxPlayerCacheLeb;
    int _wavejbmFlag;
    int _wavejbmMode;
    float _wavejbmMinSpeedRate;
    float _wavejbmMaxSpeedRate;
    int _waveVolumeEqFlag;
    float _startPlayVolume;
    float _stopPlayVolume;
    int _customRenderVal;
    int _svrLDPlayCfg;
    NSString *_itemId;
    UIView *_videoView;
    NSString *_url;
    NSString *_streamId;
    NSString *_originalUrl;
    MMLiveCdnStreamInfo *_cdnStreamInfo;
    MMLiveCDNPlayerStreamWrap *_nextStreamWrap;
    long long _renderMode;
    unsigned long long _startPlayTimestamp;
    unsigned long long _playerConnectTimestamp;
    unsigned long long _firstIFrameTimestamp;
    unsigned long long _playBeginTimestamp;
    unsigned long long _state;
    unsigned long long _lastScene;
    unsigned long long _currentScene;
    unsigned long long _preferredNetworkChannel;
    MMLiveCommonSEIMsg *_lastConnectMicSEIMsg;
    MMLiveCommonSEIMsg *_lastAnchorStatusSEIMsg;
    NSArray *_cachedMicUsersList;
    MMLiveCDNPlayerCustomExtraPram *_customExtraParam;
    NSString *_anchorNickname;
    unsigned long long _liveId;
    unsigned long long _objectId;
    CustomPlayerConfig *_customPlayerCfg;
    TRTCCdnPlayerManager *_cdnPlayer;
    NSMutableDictionary *_clarityDictionary;
    unsigned long long _lastAudioDeactiveTimestampMs;
    unsigned long long _firstAudioFrameTimestamp;
    unsigned long long _switchStreamBeginTime;
    CdnRenderViewBase *_barrageMaskRenderView;
    long long _maxSupportFps;
    id <MMLiveCDNPlayerItemDelegate> _delegate;
    MMLiveTask *_liveTask;
    long long _playerOrientation;
    unsigned long long _playerCacheType;
    NSString *_encryptKey;
    NSString *_encryptIV;
    NSString *_encryptUrl;
    unsigned long long _reportInitialScene;
    NSMutableDictionary *_sdkIntervalReportDict;
    NSMutableDictionary *_sdkEndReportDict;
    unsigned long long _videoDecodeType;
    unsigned long long _enterSessionId;
    FinderLiveVoipStreamStatus *_streamStatus;
    NSMutableDictionary *_audioNoSoundError2TimeDict;
    unsigned long long _lastReportAudioDecodeErrorTimeMs;
    struct IWaveAudioHAidInterface *_waveAudioHAidInst;
    MMLiveAudioVolumeManageLogic *_volumeManageLogic;
    struct CGSize _videoSize;
}

- (void).cxx_destruct;
@property(nonatomic) _Bool enableBackgroundRendering; // @synthesize enableBackgroundRendering=_enableBackgroundRendering;
@property(retain, nonatomic) MMLiveAudioVolumeManageLogic *volumeManageLogic; // @synthesize volumeManageLogic=_volumeManageLogic;
@property(nonatomic) struct IWaveAudioHAidInterface *waveAudioHAidInst; // @synthesize waveAudioHAidInst=_waveAudioHAidInst;
@property(nonatomic) unsigned long long lastReportAudioDecodeErrorTimeMs; // @synthesize lastReportAudioDecodeErrorTimeMs=_lastReportAudioDecodeErrorTimeMs;
@property(retain, nonatomic) NSMutableDictionary *audioNoSoundError2TimeDict; // @synthesize audioNoSoundError2TimeDict=_audioNoSoundError2TimeDict;
@property(retain, nonatomic) FinderLiveVoipStreamStatus *streamStatus; // @synthesize streamStatus=_streamStatus;
@property(nonatomic) unsigned long long enterSessionId; // @synthesize enterSessionId=_enterSessionId;
@property(nonatomic) unsigned long long videoDecodeType; // @synthesize videoDecodeType=_videoDecodeType;
@property(nonatomic) int svrLDPlayCfg; // @synthesize svrLDPlayCfg=_svrLDPlayCfg;
@property(retain, nonatomic) NSMutableDictionary *sdkEndReportDict; // @synthesize sdkEndReportDict=_sdkEndReportDict;
@property(retain, nonatomic) NSMutableDictionary *sdkIntervalReportDict; // @synthesize sdkIntervalReportDict=_sdkIntervalReportDict;
@property(nonatomic) _Bool hasEnteredBackgroundWhenWaitingPlayingOK; // @synthesize hasEnteredBackgroundWhenWaitingPlayingOK=_hasEnteredBackgroundWhenWaitingPlayingOK;
@property(nonatomic) unsigned long long reportInitialScene; // @synthesize reportInitialScene=_reportInitialScene;
@property(nonatomic) int customRenderVal; // @synthesize customRenderVal=_customRenderVal;
@property(copy, nonatomic) NSString *encryptUrl; // @synthesize encryptUrl=_encryptUrl;
@property(copy, nonatomic) NSString *encryptIV; // @synthesize encryptIV=_encryptIV;
@property(copy, nonatomic) NSString *encryptKey; // @synthesize encryptKey=_encryptKey;
@property(nonatomic) float stopPlayVolume; // @synthesize stopPlayVolume=_stopPlayVolume;
@property(nonatomic) float startPlayVolume; // @synthesize startPlayVolume=_startPlayVolume;
@property(nonatomic) int waveVolumeEqFlag; // @synthesize waveVolumeEqFlag=_waveVolumeEqFlag;
@property(nonatomic) float wavejbmMaxSpeedRate; // @synthesize wavejbmMaxSpeedRate=_wavejbmMaxSpeedRate;
@property(nonatomic) float wavejbmMinSpeedRate; // @synthesize wavejbmMinSpeedRate=_wavejbmMinSpeedRate;
@property(nonatomic) int wavejbmMode; // @synthesize wavejbmMode=_wavejbmMode;
@property(nonatomic) int wavejbmFlag; // @synthesize wavejbmFlag=_wavejbmFlag;
@property(nonatomic) _Bool hasSetPlayerCacheConfig; // @synthesize hasSetPlayerCacheConfig=_hasSetPlayerCacheConfig;
@property(nonatomic) int maxPlayerCacheLeb; // @synthesize maxPlayerCacheLeb=_maxPlayerCacheLeb;
@property(nonatomic) int minPlayerCacheLeb; // @synthesize minPlayerCacheLeb=_minPlayerCacheLeb;
@property(nonatomic) int maxPlayerCache; // @synthesize maxPlayerCache=_maxPlayerCache;
@property(nonatomic) int minPlayerCache; // @synthesize minPlayerCache=_minPlayerCache;
@property(nonatomic) unsigned long long playerCacheType; // @synthesize playerCacheType=_playerCacheType;
@property(nonatomic) _Bool hasSetPlayerCacheType; // @synthesize hasSetPlayerCacheType=_hasSetPlayerCacheType;
@property(nonatomic) _Bool hasSetPlayerRenderMode; // @synthesize hasSetPlayerRenderMode=_hasSetPlayerRenderMode;
@property(nonatomic) long long playerOrientation; // @synthesize playerOrientation=_playerOrientation;
@property(nonatomic) _Bool hasSetPlayerOrientation; // @synthesize hasSetPlayerOrientation=_hasSetPlayerOrientation;
@property(nonatomic) _Bool serverSwitchRciRendering; // @synthesize serverSwitchRciRendering=_serverSwitchRciRendering;
@property(nonatomic) __weak MMLiveTask *liveTask; // @synthesize liveTask=_liveTask;
@property(nonatomic) __weak id <MMLiveCDNPlayerItemDelegate> delegate; // @synthesize delegate=_delegate;
@property(nonatomic) long long maxSupportFps; // @synthesize maxSupportFps=_maxSupportFps;
@property(nonatomic) __weak CdnRenderViewBase *barrageMaskRenderView; // @synthesize barrageMaskRenderView=_barrageMaskRenderView;
@property(nonatomic) _Bool needCacheRecentCdnPixelBuffer; // @synthesize needCacheRecentCdnPixelBuffer=_needCacheRecentCdnPixelBuffer;
@property(nonatomic) _Bool enableBackgroundDecoding; // @synthesize enableBackgroundDecoding=_enableBackgroundDecoding;
@property(nonatomic) _Bool isToBeMuted; // @synthesize isToBeMuted=_isToBeMuted;
@property(nonatomic) unsigned long long switchStreamBeginTime; // @synthesize switchStreamBeginTime=_switchStreamBeginTime;
@property(nonatomic) unsigned long long firstAudioFrameTimestamp; // @synthesize firstAudioFrameTimestamp=_firstAudioFrameTimestamp;
@property(nonatomic) unsigned long long lastAudioDeactiveTimestampMs; // @synthesize lastAudioDeactiveTimestampMs=_lastAudioDeactiveTimestampMs;
@property(copy, nonatomic) NSMutableDictionary *clarityDictionary; // @synthesize clarityDictionary=_clarityDictionary;
@property(retain, nonatomic) TRTCCdnPlayerManager *cdnPlayer; // @synthesize cdnPlayer=_cdnPlayer;
@property(nonatomic) _Bool isGameMode; // @synthesize isGameMode=_isGameMode;
@property(nonatomic) _Bool isKTVMode; // @synthesize isKTVMode=_isKTVMode;
@property(nonatomic) _Bool isAudioRoomMode; // @synthesize isAudioRoomMode=_isAudioRoomMode;
@property(nonatomic) _Bool isAudioMode; // @synthesize isAudioMode=_isAudioMode;
@property(nonatomic) _Bool isMinimizeMode; // @synthesize isMinimizeMode=_isMinimizeMode;
@property(nonatomic) int commentScene; // @synthesize commentScene=_commentScene;
@property(retain, nonatomic) CustomPlayerConfig *customPlayerCfg; // @synthesize customPlayerCfg=_customPlayerCfg;
@property(nonatomic) unsigned long long objectId; // @synthesize objectId=_objectId;
@property(nonatomic) unsigned long long liveId; // @synthesize liveId=_liveId;
@property(copy, nonatomic) NSString *anchorNickname; // @synthesize anchorNickname=_anchorNickname;
@property(retain, nonatomic) MMLiveCDNPlayerCustomExtraPram *customExtraParam; // @synthesize customExtraParam=_customExtraParam;
@property(retain, nonatomic) NSArray *cachedMicUsersList; // @synthesize cachedMicUsersList=_cachedMicUsersList;
@property(retain, nonatomic) MMLiveCommonSEIMsg *lastAnchorStatusSEIMsg; // @synthesize lastAnchorStatusSEIMsg=_lastAnchorStatusSEIMsg;
@property(retain, nonatomic) MMLiveCommonSEIMsg *lastConnectMicSEIMsg; // @synthesize lastConnectMicSEIMsg=_lastConnectMicSEIMsg;
@property(nonatomic) unsigned long long preferredNetworkChannel; // @synthesize preferredNetworkChannel=_preferredNetworkChannel;
@property(nonatomic) _Bool isPreplayMode; // @synthesize isPreplayMode=_isPreplayMode;
@property(nonatomic) int sampleRate; // @synthesize sampleRate=_sampleRate;
@property(nonatomic) int channelCount; // @synthesize channelCount=_channelCount;
@property(nonatomic) _Bool shouldStartInBackground; // @synthesize shouldStartInBackground=_shouldStartInBackground;
@property(nonatomic) _Bool interruptedWhenAppResignActive; // @synthesize interruptedWhenAppResignActive=_interruptedWhenAppResignActive;
@property(nonatomic) int encryptMode; // @synthesize encryptMode=_encryptMode;
@property(nonatomic) int liveVrType; // @synthesize liveVrType=_liveVrType;
@property(nonatomic) _Bool isPlayingLocalAudioLiveStream; // @synthesize isPlayingLocalAudioLiveStream=_isPlayingLocalAudioLiveStream;
@property(nonatomic) unsigned long long currentScene; // @synthesize currentScene=_currentScene;
@property(nonatomic) unsigned long long lastScene; // @synthesize lastScene=_lastScene;
@property(nonatomic) unsigned long long state; // @synthesize state=_state;
@property(nonatomic) unsigned long long playBeginTimestamp; // @synthesize playBeginTimestamp=_playBeginTimestamp;
@property(nonatomic) unsigned long long firstIFrameTimestamp; // @synthesize firstIFrameTimestamp=_firstIFrameTimestamp;
@property(nonatomic) unsigned long long playerConnectTimestamp; // @synthesize playerConnectTimestamp=_playerConnectTimestamp;
@property(nonatomic) unsigned long long startPlayTimestamp; // @synthesize startPlayTimestamp=_startPlayTimestamp;
@property(nonatomic) _Bool hasConnectedSuccess; // @synthesize hasConnectedSuccess=_hasConnectedSuccess;
@property(nonatomic) _Bool hasReceivedFirstIFrame; // @synthesize hasReceivedFirstIFrame=_hasReceivedFirstIFrame;
@property(nonatomic) _Bool hasPlayBegan; // @synthesize hasPlayBegan=_hasPlayBegan;
@property(nonatomic) struct CGSize videoSize; // @synthesize videoSize=_videoSize;
@property(nonatomic) long long renderMode; // @synthesize renderMode=_renderMode;
@property(retain, nonatomic) MMLiveCDNPlayerStreamWrap *nextStreamWrap; // @synthesize nextStreamWrap=_nextStreamWrap;
@property(retain, nonatomic) MMLiveCdnStreamInfo *cdnStreamInfo; // @synthesize cdnStreamInfo=_cdnStreamInfo;
@property(copy, nonatomic) NSString *originalUrl; // @synthesize originalUrl=_originalUrl;
@property(copy, nonatomic) NSString *streamId; // @synthesize streamId=_streamId;
@property(copy, nonatomic) NSString *url; // @synthesize url=_url;
@property(retain, nonatomic) UIView *videoView; // @synthesize videoView=_videoView;
@property(copy, nonatomic) NSString *itemId; // @synthesize itemId=_itemId;
- (void)clartyDictionaryAddWithStreamId:(id)arg1 cdnTransInfo:(id)arg2;
- (void)onPlayEventAddCustomParamWithEvent:(int)arg1 changeParam:(id)arg2;
- (void)replaceAudioNoSoundError:(id)arg1 count:(unsigned int)arg2;
- (void)appendAudioNoSoundError:(id)arg1 count:(unsigned int)arg2;
- (void)addSharedValueToMDict:(id)arg1;
- (void)fillParamForSDKPlayReportWithIntervalDict:(id)arg1 endDict:(id)arg2;
- (void)refillSDKReport;
- (void)updateSDKReportWithAnchorSEIStatus:(unsigned int)arg1 delayInMs:(unsigned int)arg2;
- (void)setupStreamStatusForReport;
- (void)wcAVAudioSessionSwizzleDidCallMethod:(id)arg1 error:(id)arg2 ret:(_Bool)arg3 caller:(id)arg4;
- (void)onWCAudioSessionCategoryChange;
- (void)onWCAudioSessionSetActive:(_Bool)arg1;
- (_Bool)enableLiveAudioHAid;
- (void)createWaveAudioHAidInstWithSampleRate:(int)arg1 channels:(int)arg2;
- (void)onPcmDataAvailable:(id)arg1 pts:(unsigned long long)arg2;
- (void)onAudioInfoChanged:(int)arg1 channels:(int)arg2;
- (void)setEnhanceOptions:(unsigned long long)arg1;
- (unsigned long long)enhanceOptions;
- (void)appendCustomerPlayReportInterval:(id)arg1;
- (void)appendCustomerPlayReportEnd:(id)arg1;
- (void)appendCustomerPlayReportBegin:(id)arg1;
- (void)setCDNPlayDecryption:(id)arg1 encMode:(unsigned int)arg2 encKey:(id)arg3 encIV:(id)arg4;
- (void)verifyAndSetDecryptionParams:(id)arg1;
- (void)setCDNPlayDecryptionParams:(id)arg1 encMode:(unsigned int)arg2 encKey:(id)arg3 encIV:(id)arg4;
- (void)setCustomRenderParam:(id)arg1;
- (void)onCdnRenderFpsUpdate:(int)arg1;
- (void)onOutputWeEffectProcessedRenderFrame:(id)arg1;
- (id)getRootLiveTask;
- (_Bool)isCDNPlayerNeedStopRenderPixel;
- (void)onPlayerPixelBufferRendered:(struct __CVBuffer *)arg1;
- (void)onPlayerPixelBufferRefresh:(struct __CVBuffer *)arg1;
@property(readonly, nonatomic) long long customRenderMode;
- (int)getSharpenInfo;
- (int)getCustomRenderMode;
- (int)getCustomRenderEnable;
- (void)resetVideoZoomAndYaw;
- (void)setSensorControlFlag:(_Bool)arg1 keepLastSensor:(_Bool)arg2;
- (void)setTouchControlFlag:(_Bool)arg1;
- (void)handleGesturesOperation:(id)arg1;
- (int)switchStreamWithUrl:(id)arg1 originalUrl:(id)arg2 streamInfo:(id)arg3 trtcChannelParams:(id)arg4 switchByUser:(_Bool)arg5;
- (int)switchStreamWithUrl:(id)arg1 originalUrl:(id)arg2 streamInfo:(id)arg3;
- (int)switchStreamWithUrl:(id)arg1;
- (void)refreshCdnPlayerRciEnable;
- (_Bool)computeRciEnable;
- (void)configRciRenderingWithLiveSdkParams:(id)arg1;
- (void)configSuperResolution:(_Bool)arg1;
- (void)configAdaptiveVolumeEQ:(unsigned long long)arg1;
- (void)setCacheConfig:(unsigned int)arg1 wavejbmMode:(unsigned int)arg2 wavejbmMinSpeedRate:(double)arg3 wavejbmMaxSpeedRate:(double)arg4;
- (void)setGameAppId:(id)arg1;
- (void)setCacheType:(unsigned long long)arg1 minCache:(int)arg2 maxCache:(int)arg3 lebMinCache:(int)arg4 lebMaxCache:(int)arg5;
- (void)setOrientation:(long long)arg1;
- (void)clearCachedMicUsersList;
- (void)updateMaxSupportFps:(long long)arg1;
- (unsigned long long)currVideoDecodeType;
- (_Bool)enableRci;
- (_Bool)needProcessByWeffect;
- (void)updateNeedProcessByWeffect:(_Bool)arg1;
- (void)configCdnWeEffectManager:(id)arg1;
- (id)getCdnRenderContext;
- (void)updateNeedCacheRecentCdnPixelBuffer:(_Bool)arg1;
- (void)updateBarrageMaskRenderView:(id)arg1;
- (struct __CVBuffer *)getCurrentLivePixelBufferRef;
- (_Bool)isCurrentEnableCdnRenderPixelBuffer;
- (void)enableCdnRenderPixelBuffer;
- (void)disableCdnRenderPixelBufferWithForbidPixel:(_Bool)arg1 hideView:(_Bool)arg2;
@property(readonly, nonatomic) float currentVideoDisplayScale;
- (void)requestYUVColorAtNormalizedCoordinateList:(id)arg1 fromRectWithSize:(struct CGSize)arg2 callback:(CDUnknownBlockType)arg3;
- (void)requestARGBColorAtNormalizedCoordinateList:(id)arg1 fromRectWithSize:(struct CGSize)arg2 callback:(CDUnknownBlockType)arg3;
- (void)enableBackgroundRendering:(_Bool)arg1;
- (void)enableBackgroundDecoding:(_Bool)arg1;
- (void)unSetVolume:(long long)arg1;
- (void)setVolume:(long long)arg1 business:(long long)arg2;
- (void)setMute:(_Bool)arg1;
- (_Bool)isPlaying;
- (void)resume;
- (void)resetRenderStats;
- (void)pause;
- (void)stopPlay;
- (void)delayStopPlayingForPreload;
- (void)switchToPlayingForPreloadWithDelayStopTime:(double)arg1;
- (_Bool)startPlayForPreloadWithUrl:(id)arg1 originalUrl:(id)arg2 isLocalAudioLiveStream:(_Bool)arg3 andDelayStopTime:(double)arg4;
- (_Bool)startPlayForPreloadWithUrl:(id)arg1 andDelayStopTime:(double)arg2;
- (void)switchToPlaying;
- (void)resetTRTCCdnPlayerDefaultParams;
- (_Bool)needBackgroundDecoding;
- (_Bool)backgroundDecodingSwitch;
- (void)resetTRTCCdnPlayerParams;
- (void)cdnPlayerStartPlayWithUrl:(id)arg1 isMuted:(_Bool)arg2 forceRecreateCdnPlayer:(_Bool)arg3;
- (void)cdnPlayerStartPlayWithUrl:(id)arg1 isMuted:(_Bool)arg2;
- (void)callExperimentalAPI:(id)arg1;
- (void)updateStreamInfo:(id)arg1;
- (void)startPlayWithUrl:(id)arg1 originalUrl:(id)arg2 streamInfo:(id)arg3 forceRecreateCdnPlayer:(_Bool)arg4;
- (void)startPlayWithUrl:(id)arg1 originalUrl:(id)arg2 streamInfo:(id)arg3;
- (void)startPlayWithUrl:(id)arg1 andOriginalUrl:(id)arg2;
- (void)startPlayWithUrl:(id)arg1;
@property(readonly, nonatomic) unsigned int currentQualityTag;
- (_Bool)isSameStreamIdFromUrl:(id)arg1;
- (void)clearDelegate;
- (void)onNetStatus:(id)arg1;
- (void)tryReportStreamStatus;
- (void)tryReportPlayStatus;
@property(readonly, nonatomic) unsigned long long playStatus;
- (unsigned int)playTime;
- (void)tryReportNoSoundError;
- (void)tryReportVolumeStatus;
- (void)tryReportPlayTimeCostLog;
- (void)debug_autoChangeVideoDecodeType;
- (void)updateVideoDecodeType:(unsigned long long)arg1;
- (void)onPlayEvent:(int)arg1 withParam:(id)arg2;
- (void)tryFixUnknowHostErr;
- (void)destroyCustomExtraParam;
- (void)resetPlayEventValues:(_Bool)arg1;
@property(readonly, nonatomic) _Bool isAudioUnitInterrupted;
@property(readonly, nonatomic) _Bool isLiveRoomPreparedItem;
@property(readonly, nonatomic) _Bool isInLiveRoom;
@property(readonly, nonatomic) _Bool isMuted;
@property(readonly, nonatomic) _Bool isSmallScaleDisplayMode;
@property(readonly, nonatomic) unsigned long long currentLiveType;
- (void)onEnterBackground;
@property(retain, nonatomic) UIView *videoViewProbe;
- (void)dealloc;
- (id)initWithItemId:(id)arg1 delegate:(id)arg2 playScene:(unsigned long long)arg3 liveVrType:(int)arg4;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
// Preceding property had unknown attributes: ?
// Original attribute string: T@"NSString",?,R,C

@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

