//
//     Generated by class-dump 3.5 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard.
//

@class ContactTagListBottomBar, ContactTagSearchFromContactLogic, MMPageSheetAdapter, MMTableView, NSArray, NSMutableArray, NSMutableDictionary, NSMutableOrderedSet, NSMutableSet, NSString, UILabel, WCMomentsContactTagEditView, WCMomentsContactTagSearchViewController, WCSearchBar;
@protocol WCMomentsContactTagListViewControllerDelegate;

@interface WCMomentsContactTagListViewController
{
    ContactTagSearchFromContactLogic *m_contactTagSearchLogic;
    WCMomentsContactTagSearchViewController *m_searchVC;
    _Bool m_needReload;
    _Bool m_forbidSelectNotMemberTag;
    _Bool m_stopSoredSelectContactTag;
    _Bool _canEditTagLabel;
    _Bool _hideSearchBar;
    _Bool _enableConfirmAfterModify;
    _Bool _embeddedInContainer;
    _Bool _hasModifiedSelectedContacts;
    _Bool _isInlineSearching;
    id <WCMomentsContactTagListViewControllerDelegate> _delegate;
    NSString *_viewControllerTitle;
    NSString *_rightBarTitle;
    long long _privacy;
    long long _enterScene;
    unsigned long long _sourceScene;
    NSMutableOrderedSet *_selectedTagIdSet;
    NSArray *_requiredTags;
    NSArray *_disabledTags;
    MMPageSheetAdapter *_pageSheetAdapter;
    CDUnknownBlockType _selectedTagsDidChangeBlock;
    MMTableView *_tableView;
    NSMutableDictionary *_tagDataCache;
    NSMutableArray *_arrAllTagID;
    UILabel *_noTagTips;
    WCSearchBar *_searchBar;
    WCMomentsContactTagEditView *_tagEditView;
    NSMutableDictionary *_searchResultsDataCache;
    NSMutableSet *_searchSelectedTagIDSet;
    ContactTagListBottomBar *_bottomBar;
    MMTableView *_inlineSearchTableView;
    NSArray *_inlineSearchResults;
    NSString *_lastInlineSearchQuery;
    NSString *_reportSessionId;
}

- (void).cxx_destruct;
@property(retain, nonatomic) NSString *reportSessionId; // @synthesize reportSessionId=_reportSessionId;
@property(copy, nonatomic) NSString *lastInlineSearchQuery; // @synthesize lastInlineSearchQuery=_lastInlineSearchQuery;
@property(nonatomic) _Bool isInlineSearching; // @synthesize isInlineSearching=_isInlineSearching;
@property(retain, nonatomic) NSArray *inlineSearchResults; // @synthesize inlineSearchResults=_inlineSearchResults;
@property(retain, nonatomic) MMTableView *inlineSearchTableView; // @synthesize inlineSearchTableView=_inlineSearchTableView;
@property(retain, nonatomic) ContactTagListBottomBar *bottomBar; // @synthesize bottomBar=_bottomBar;
@property(nonatomic) _Bool hasModifiedSelectedContacts; // @synthesize hasModifiedSelectedContacts=_hasModifiedSelectedContacts;
@property(retain, nonatomic) NSMutableSet *searchSelectedTagIDSet; // @synthesize searchSelectedTagIDSet=_searchSelectedTagIDSet;
@property(retain, nonatomic) NSMutableDictionary *searchResultsDataCache; // @synthesize searchResultsDataCache=_searchResultsDataCache;
@property(retain, nonatomic) WCMomentsContactTagEditView *tagEditView; // @synthesize tagEditView=_tagEditView;
@property(retain, nonatomic) WCSearchBar *searchBar; // @synthesize searchBar=_searchBar;
@property(retain, nonatomic) UILabel *noTagTips; // @synthesize noTagTips=_noTagTips;
@property(retain, nonatomic) NSMutableArray *arrAllTagID; // @synthesize arrAllTagID=_arrAllTagID;
@property(retain, nonatomic) NSMutableDictionary *tagDataCache; // @synthesize tagDataCache=_tagDataCache;
@property(retain, nonatomic) MMTableView *tableView; // @synthesize tableView=_tableView;
@property(copy, nonatomic) CDUnknownBlockType selectedTagsDidChangeBlock; // @synthesize selectedTagsDidChangeBlock=_selectedTagsDidChangeBlock;
@property(nonatomic) _Bool embeddedInContainer; // @synthesize embeddedInContainer=_embeddedInContainer;
@property(nonatomic) __weak MMPageSheetAdapter *pageSheetAdapter; // @synthesize pageSheetAdapter=_pageSheetAdapter;
@property(retain, nonatomic) NSArray *disabledTags; // @synthesize disabledTags=_disabledTags;
@property(retain, nonatomic) NSArray *requiredTags; // @synthesize requiredTags=_requiredTags;
@property(retain, nonatomic) NSMutableOrderedSet *selectedTagIdSet; // @synthesize selectedTagIdSet=_selectedTagIdSet;
@property(nonatomic) unsigned long long sourceScene; // @synthesize sourceScene=_sourceScene;
@property(nonatomic) long long enterScene; // @synthesize enterScene=_enterScene;
@property(nonatomic) long long privacy; // @synthesize privacy=_privacy;
@property(nonatomic) _Bool enableConfirmAfterModify; // @synthesize enableConfirmAfterModify=_enableConfirmAfterModify;
@property(nonatomic) _Bool hideSearchBar; // @synthesize hideSearchBar=_hideSearchBar;
@property(nonatomic) _Bool canEditTagLabel; // @synthesize canEditTagLabel=_canEditTagLabel;
@property(retain, nonatomic) NSString *rightBarTitle; // @synthesize rightBarTitle=_rightBarTitle;
@property(retain, nonatomic) NSString *viewControllerTitle; // @synthesize viewControllerTitle=_viewControllerTitle;
@property(nonatomic) __weak id <WCMomentsContactTagListViewControllerDelegate> delegate; // @synthesize delegate=_delegate;
- (void)inlineSearchDidSelectRowAtIndexPath:(id)arg1;
- (id)inlineSearchCellForRowAtIndexPath:(id)arg1 inTableView:(id)arg2;
- (long long)inlineSearchTableViewNumberOfRows;
- (void)onInlineSearchTextFieldChanged:(id)arg1;
- (void)mmInputBoxClear;
- (void)mmInputBoxTextChanged:(id)arg1 text:(id)arg2;
- (void)performInlineSearchWithQuery:(id)arg1;
- (void)hideInlineSearchResults;
- (void)showInlineSearchResults;
- (void)initInlineSearchTableView;
- (void)reloadTagList;
- (void)onUpdateLabelOrder:(id)arg1 withRetCode:(int)arg2 andEventId:(unsigned int)arg3;
- (void)OnMakeSureUpdateContactTagName;
- (void)OnMakeSureDelteContactTag:(id)arg1;
- (_Bool)handleQuitEditMode:(id)arg1;
- (void)OnNewContactTagCreateTagWithTagId:(unsigned int)arg1 result:(_Bool)arg2 isRename:(_Bool)arg3;
- (void)onContactTagListBottomBarDelete;
- (void)onContactTagListBottomBarMoveTo;
- (void)onContactTagListBottomBarEditTags;
- (void)onContactTagListBottomBarCreateTag;
- (id)getAllContactUserNameMatchTip;
- (void)onContactAsynContactTagResultChanged;
- (void)contactTagEditViewDidClickSearch:(id)arg1;
- (void)contactTagEditView:(id)arg1 deleteTagData:(id)arg2;
- (void)onSearchViewControllerDidCancelItemClick;
- (_Bool)isTagName:(id)arg1 containsQuery:(id)arg2;
- (id)searchViewController:(id)arg1 searchWithQuery:(id)arg2;
- (void)searchViewController:(id)arg1 didSelectTagDataSet:(id)arg2 multiSelectedFlag:(_Bool)arg3;
- (void)onTagDetailDismissedWithAddContactLabel;
- (void)reloadContactTagListView;
- (void)makeNewContactTag;
- (void)onSaveBarButtonPress:(id)arg1;
- (void)onDismissBarButtonPress:(id)arg1;
- (void)clearCache;
- (void)reloadTableView;
- (void)reloadData;
- (_Bool)canShowTopBar;
- (void)deleteSelectedTagData:(id)arg1;
- (void)addSelectedTagData:(id)arg1;
- (void)addSelectedTagDatas:(id)arg1;
- (id)tagDatasWithTagIDs:(id)arg1;
- (id)tagDataWithTagID:(id)arg1;
- (id)tagDataAtIndexPath:(id)arg1;
- (_Bool)isTagSelected:(id)arg1;
- (_Bool)shouldForbidSelectEmptyTag:(id)arg1;
- (_Bool)isTagSelectEnabled:(id)arg1;
- (_Bool)filterTag:(id)arg1;
- (void)showContactTagDetailViewWithContactTagData:(id)arg1;
- (void)showSearchVC;
- (void)updateRightBarButtonItemIfNeeded;
- (id)rightBarButtonItemWithTitle:(id)arg1;
- (id)rightBarButtonItemWithSelectTagCount:(unsigned long long)arg1 prefix:(id)arg2;
- (id)getRightBarButtonItem;
- (id)getLeftBarButtonItem;
- (id)manageTagCellForTableView:(id)arg1;
- (id)createTagCellForTableView:(id)arg1;
- (void)tableView:(id)arg1 didSelectRowAtIndexPath:(id)arg2;
- (id)tableView:(id)arg1 cellForRowAtIndexPath:(id)arg2;
- (double)tableView:(id)arg1 heightForRowAtIndexPath:(id)arg2;
- (long long)tableView:(id)arg1 numberOfRowsInSection:(long long)arg2;
- (long long)numberOfSectionsInTableView:(id)arg1;
- (void)viewWillAppear:(_Bool)arg1;
- (void)viewDidBePopedOrDismissed:(_Bool)arg1;
- (void)viewDidBePushOrPresent:(_Bool)arg1;
- (void)viewDidLayoutSubviews;
- (void)viewDidLoad;
- (id)createSearchVC;
- (void)initBottomBar;
- (double)getContentViewY;
- (void)updateNoTagTipsFrame;
- (void)updateViewDisplayStateWithTagData:(id)arg1;
- (void)initNoTagTips;
- (void)switchTopBarIfNeeded;
- (void)updateTopBar;
- (void)initTableView;
- (void)initData;
- (void)dealloc;
- (id)init;
- (id)getReportDictionary;
- (unsigned int)getBusinessTypeWithSourceScene;
- (void)reportClickFinishDeleteContactTagButton:(unsigned int)arg1;
- (void)reportClickFinishUpdateContactTagNameButton;
- (void)reportClickContactTagManageButton;
- (void)reportClickFinishContactTagCreateButton:(id)arg1;
- (void)reportClickContactTagCreateButton;
- (void)setupReportData;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
// Preceding property had unknown attributes: ?
// Original attribute string: T@"NSString",?,R,C

@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

