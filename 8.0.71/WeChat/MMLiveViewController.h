//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

@class MMFinderCameraCastingStarterViewController, MMFinderLiveNewStartViewController, MMFinderLiveSquareChatStartViewController, MMLiveCompleteViewController, MMLiveConnectMicLogic, MMLiveLogic, MMLiveOperationView, MMLivePivotViewController, MMLiveStarterViewController, MMLiveTask, MMLiveTaskId, MMLiveView, NSMutableDictionary, NSString, UIImageView, UINavigationController, UIView, UIViewController;
@protocol MMLiveContainerDisplayViewProtocol, MMLiveViewControllerDelegate;

@interface MMLiveViewController
{
    _Bool _isForcePortraitForExtend;
    _Bool _isPresentingWithCustomTransition;
    _Bool _liveDidFinished;
    _Bool _isEmbededInLiveFeed;
    _Bool _isEmbededInLiveFeedByReusing;
    _Bool _isExitingEmbededLiveFromCloseUI;
    _Bool _isClosing;
    _Bool _waitingJoinLive;
    _Bool _postponeJoinLive;
    _Bool _supressMinimizeWindowAutoupdate;
    _Bool _isAppTaskLaunchingDuringLive;
    _Bool _isInBackground;
    _Bool _isRealInBackground;
    _Bool _isInSysInterruptionHandling;
    _Bool _isPausedByAnchor;
    _Bool _isPausedByVerify;
    _Bool _anchorJoinLiveAfterAbnormalExit;
    _Bool _isAbnormallyDismissed;
    _Bool _liveDidStarted;
    _Bool _isLiveAppearing;
    id <MMLiveViewControllerDelegate> _delegate;
    MMLiveView *_liveView;
    MMLiveTaskId *_liveTaskId;
    MMLiveLogic *_liveLogic;
    MMLiveCompleteViewController *_completeViewController;
    UIImageView *_blurView;
    MMLiveStarterViewController *_starterViewController;
    MMFinderLiveNewStartViewController *_startNewViewController;
    MMFinderLiveSquareChatStartViewController *_squareChatStartViewController;
    MMFinderCameraCastingStarterViewController *_cameraCastingViewController;
    UINavigationController *_navControl;
    UIViewController *_parentFeedListViewController;
    UIView *_feedLiveRoomParentView;
    CDUnknownBlockType _closeLiveBlock;
    NSString *_lastNetworkType;
    CDUnknownBlockType _forbidLiveActionCallback;
    NSMutableDictionary *_completeExtraInfo;
    unsigned long long _currSupportedInterfaceOrientations;
    MMLiveOperationView *_operationView;
    long long _currOrientation;
    unsigned long long _currentPauseViewTypes;
}

- (void).cxx_destruct;
@property(readonly, nonatomic) unsigned long long currentPauseViewTypes; // @synthesize currentPauseViewTypes=_currentPauseViewTypes;
@property(nonatomic) _Bool isLiveAppearing; // @synthesize isLiveAppearing=_isLiveAppearing;
@property(nonatomic) _Bool liveDidStarted; // @synthesize liveDidStarted=_liveDidStarted;
@property(nonatomic) _Bool isAbnormallyDismissed; // @synthesize isAbnormallyDismissed=_isAbnormallyDismissed;
@property(nonatomic) _Bool anchorJoinLiveAfterAbnormalExit; // @synthesize anchorJoinLiveAfterAbnormalExit=_anchorJoinLiveAfterAbnormalExit;
@property(nonatomic) _Bool isPausedByVerify; // @synthesize isPausedByVerify=_isPausedByVerify;
@property(nonatomic) _Bool isPausedByAnchor; // @synthesize isPausedByAnchor=_isPausedByAnchor;
@property(nonatomic) _Bool isInSysInterruptionHandling; // @synthesize isInSysInterruptionHandling=_isInSysInterruptionHandling;
@property(nonatomic) _Bool isRealInBackground; // @synthesize isRealInBackground=_isRealInBackground;
@property(nonatomic) _Bool isInBackground; // @synthesize isInBackground=_isInBackground;
@property(nonatomic) long long currOrientation; // @synthesize currOrientation=_currOrientation;
@property(retain, nonatomic) MMLiveOperationView *operationView; // @synthesize operationView=_operationView;
@property(nonatomic) unsigned long long currSupportedInterfaceOrientations; // @synthesize currSupportedInterfaceOrientations=_currSupportedInterfaceOrientations;
@property(nonatomic) _Bool isAppTaskLaunchingDuringLive; // @synthesize isAppTaskLaunchingDuringLive=_isAppTaskLaunchingDuringLive;
@property(nonatomic) _Bool supressMinimizeWindowAutoupdate; // @synthesize supressMinimizeWindowAutoupdate=_supressMinimizeWindowAutoupdate;
@property(nonatomic) _Bool postponeJoinLive; // @synthesize postponeJoinLive=_postponeJoinLive;
@property(retain, nonatomic) NSMutableDictionary *completeExtraInfo; // @synthesize completeExtraInfo=_completeExtraInfo;
@property(copy, nonatomic) CDUnknownBlockType forbidLiveActionCallback; // @synthesize forbidLiveActionCallback=_forbidLiveActionCallback;
@property(retain, nonatomic) NSString *lastNetworkType; // @synthesize lastNetworkType=_lastNetworkType;
@property(nonatomic) _Bool waitingJoinLive; // @synthesize waitingJoinLive=_waitingJoinLive;
@property(nonatomic) _Bool isClosing; // @synthesize isClosing=_isClosing;
@property(copy, nonatomic) CDUnknownBlockType closeLiveBlock; // @synthesize closeLiveBlock=_closeLiveBlock;
@property(nonatomic) __weak UIView *feedLiveRoomParentView; // @synthesize feedLiveRoomParentView=_feedLiveRoomParentView;
@property(nonatomic) _Bool isExitingEmbededLiveFromCloseUI; // @synthesize isExitingEmbededLiveFromCloseUI=_isExitingEmbededLiveFromCloseUI;
@property(retain, nonatomic) UIViewController *parentFeedListViewController; // @synthesize parentFeedListViewController=_parentFeedListViewController;
@property(nonatomic) _Bool isEmbededInLiveFeedByReusing; // @synthesize isEmbededInLiveFeedByReusing=_isEmbededInLiveFeedByReusing;
@property(nonatomic) _Bool isEmbededInLiveFeed; // @synthesize isEmbededInLiveFeed=_isEmbededInLiveFeed;
@property(nonatomic) _Bool liveDidFinished; // @synthesize liveDidFinished=_liveDidFinished;
@property(nonatomic) __weak UINavigationController *navControl; // @synthesize navControl=_navControl;
@property(retain, nonatomic) MMFinderCameraCastingStarterViewController *cameraCastingViewController; // @synthesize cameraCastingViewController=_cameraCastingViewController;
@property(retain, nonatomic) MMFinderLiveSquareChatStartViewController *squareChatStartViewController; // @synthesize squareChatStartViewController=_squareChatStartViewController;
@property(retain, nonatomic) MMFinderLiveNewStartViewController *startNewViewController; // @synthesize startNewViewController=_startNewViewController;
@property(retain, nonatomic) MMLiveStarterViewController *starterViewController; // @synthesize starterViewController=_starterViewController;
@property(nonatomic) _Bool isPresentingWithCustomTransition; // @synthesize isPresentingWithCustomTransition=_isPresentingWithCustomTransition;
@property(nonatomic) _Bool isForcePortraitForExtend; // @synthesize isForcePortraitForExtend=_isForcePortraitForExtend;
@property(retain, nonatomic) UIImageView *blurView; // @synthesize blurView=_blurView;
@property(retain, nonatomic) MMLiveCompleteViewController *completeViewController; // @synthesize completeViewController=_completeViewController;
@property(retain, nonatomic) MMLiveLogic *liveLogic; // @synthesize liveLogic=_liveLogic;
@property(retain, nonatomic) MMLiveTaskId *liveTaskId; // @synthesize liveTaskId=_liveTaskId;
@property(retain, nonatomic) MMLiveView *liveView; // @synthesize liveView=_liveView;
@property(nonatomic) __weak id <MMLiveViewControllerDelegate> delegate; // @synthesize delegate=_delegate;
- (void)updateCameraCastLiveRoom;
- (void)onMakeLiveViewControllerVisible;
- (void)onCurrAnchorMainShouldSupportOrientationMaskChanged:(unsigned long long)arg1;
- (_Bool)shouldForcePortrait;
- (void)handleForEndSysInterruption;
- (void)handleForBeginSysInterruption;
- (void)onAppTaskDidRestoreIntermediatePageWhenLaunchWithAppId:(id)arg1;
- (void)onAppTaskWillRestoreIntermediatePageWhenLaunchWithAppId:(id)arg1;
- (void)onMMLiveFullScreenTipsViewClose;
- (void)onClickMakeSureActionWithViewController:(id)arg1;
- (void)onEnterForeground;
- (void)onEnterBackground;
- (void)onResignActive;
- (void)onBecomeActive;
- (void)showTipWhenChangeNetToSimCard4G:(id)arg1;
- (void)isNeedTipOnMobilePhoneTraffic;
- (void)onNetworkStatusChange:(unsigned int)arg1;
- (void)onClickTipsBtn:(id)arg1 Index:(long long)arg2;
- (void)showAnchorAssistantLiveCloseTip;
- (void)actionSheet:(id)arg1 clickedButtonAtIndex:(long long)arg2;
- (void)stopStarterViewTimer;
- (void)showForbidLiveViewWithTitleLabel:(id)arg1 withDescLabel:(id)arg2 hasLinkLabel:(_Bool)arg3 andActionCallback:(CDUnknownBlockType)arg4;
- (void)showForbidLiveViewWithTitleLabel:(id)arg1 withDescLabel:(id)arg2 andActionCallback:(CDUnknownBlockType)arg3;
- (void)handleUIOrientationChanged:(id)arg1;
- (void)showToastWithTitle:(id)arg1;
- (void)onLiveTaskMinimizedWindowDismiss:(id)arg1;
- (void)onExtendFromCollapseAnimationDidEndWithLiveTask:(id)arg1;
- (void)onMMLiveCompleteViewControllerClose;
- (void)onMMLiveOperationKTVSongButtonClicked;
- (void)onMMLiveOperationSwitchAudioRoomMode;
- (void)onMMLiveOperationInviteFriendsBtnClicked;
- (void)onMMLiveGloryTipsHeadImageClicked;
- (void)onMMLiveGlorySelfHeadImageClicked;
- (void)onMMLiveOperationUserHeadImageClicked:(id)arg1 userInfo:(id)arg2;
- (double)getControllerVisibleHeight;
- (_Bool)onMMLiveOperationIsShareScreenBlocked;
- (void)onMMLiveOperationShareScreen;
- (void)onMMLiveOperationConnectSwitchCamera;
- (void)onMMLiveOperationConnectMic;
- (void)invokeCloseLive;
- (void)onMMLiveOperationClose;
- (void)onLiveSquareChatStartViewControllerClose;
- (void)onLiveSquareChatStartViewControllerStartLive;
- (void)onLiveSquareChatStartViewControllerCreateLive:(id)arg1;
- (void)onLiveSquareChatStartViewControllerUserCancelLive;
- (void)onLiveNewStartViewControllerUserCancelLive;
- (void)onLiveNewStartViewControllerClose;
- (void)onLiveNewStartViewControllerStartLive;
- (void)onLiveNewStartViewControllerCreateLive:(id)arg1;
- (void)onMMLiveStarterViewControllerUserCancelLive;
- (void)onMMLiveStarterViewControllerSwitchCamera;
- (void)onMMLiveStarterViewControllerClose;
- (void)onMMLiveStarterViewControllerStartLive;
- (void)onMMLiveStarterViewControllerCreateLive:(id)arg1;
- (void)onLiveTask:(id)arg1 cameraCastConnectionChanged:(_Bool)arg2;
- (void)onLiveTaskMicDidReady:(id)arg1;
- (void)onLiveTaskSendFirstLocalAudioFrame:(id)arg1;
- (void)onLiveTask:(id)arg1 isAssistantValueChanged:(_Bool)arg2;
- (void)onLiveTask:(id)arg1 playingViaCdnChanged:(_Bool)arg2;
- (void)onLiveTask:(id)arg1 didChangeViewCapturing:(_Bool)arg2;
- (void)onLiveTask:(id)arg1 micMutedDidChange:(_Bool)arg2;
- (void)onLiveTask:(id)arg1 otherConnectMicAnchor:(id)arg2 anchorSeiStatus:(unsigned int)arg3 anchorSEIInfo:(id)arg4;
- (void)onLiveTask:(id)arg1 liveViewConnectMicStateChanged:(_Bool)arg2;
- (void)onLiveTask:(id)arg1 anchorIsTalking:(_Bool)arg2;
- (void)onLiveTask:(id)arg1 flvNetworkQualityUpdated:(long long)arg2 hasChanged:(_Bool)arg3;
- (void)onLiveTask:(id)arg1 liveStreamServerTimeUpdated:(unsigned long long)arg2;
- (void)onLiveTask:(id)arg1 audioRoomSEIMicMsgInfoUpdated:(id)arg2 listChanged:(_Bool)arg3;
- (void)onLiveTask:(id)arg1 audioRoomSceneChangedTo:(unsigned long long)arg2;
- (void)onLiveTask:(id)arg1 localAudioModeStateChangedTo:(unsigned long long)arg2;
- (void)onLiveTask:(id)arg1 isAudienceLocalAudioModeChangedTo:(_Bool)arg2 isPureAudioDisplayModeValueChanged:(_Bool)arg3;
- (void)onLiveTask:(id)arg1 isPureAudioModeChangedTo:(_Bool)arg2 isPureAudioDisplayModeValueChanged:(_Bool)arg3 showTip:(_Bool)arg4;
- (void)onLiveTask:(id)arg1 userVoiceVolume:(id)arg2 totalVolume:(long long)arg3 isSelfAnchorUserTalking:(_Bool)arg4;
- (void)onLiveTask:(id)arg1 SEIMicTalkingUsersListUpdated:(id)arg2;
- (void)onLiveTask:(id)arg1 SEIMicMsgInfoUpdated:(id)arg2 listChanged:(_Bool)arg3;
- (void)onLiveTask:(id)arg1 SEIConnectMicUserInfoListUpdated:(id)arg2 listChanged:(_Bool)arg3;
- (void)onLiveTask:(id)arg1 sysCallInterrupted:(_Bool)arg2;
- (void)onLiveTask:(id)arg1 connectOtherRoomResult:(_Bool)arg2 userId:(id)arg3 errCode:(int)arg4 errMsg:(id)arg5;
- (void)onLiveTask:(id)arg1 remoteUserLeaveRoom:(id)arg2 reason:(long long)arg3;
- (void)onLiveTask:(id)arg1 remoteUserEnterRoom:(id)arg2;
- (void)onLiveTask:(id)arg1 cdnPlayRemoteVideoSizeChanged:(struct CGSize)arg2;
- (void)onLiveTaskFirstShowCdnPlayerView:(id)arg1;
- (void)onLiveTask:(id)arg1 cdnPlayEventId:(int)arg2 params:(id)arg3;
- (void)onLiveTask:(id)arg1 netStatus:(id)arg2;
- (void)onLiveTask:(id)arg1 networkLocalQuality:(id)arg2 remoteQuality:(id)arg3;
- (void)onLiveTask:(id)arg1 user:(id)arg2 videoAvailable:(_Bool)arg3;
- (void)onLiveTask:(id)arg1 user:(id)arg2 audioAvailable:(_Bool)arg3;
- (void)onLiveTask:(id)arg1 switchRoleType:(int)arg2 errMsg:(id)arg3;
- (void)onLiveTask:(id)arg1 liveSdkInfoWillExpire:(id)arg2;
- (void)onLiveTask:(id)arg1 exitLiveRoom:(long long)arg2 error:(id)arg3;
- (void)onLiveTask:(id)arg1 enterLiveRoom:(id)arg2;
- (_Bool)checkTask:(id)arg1;
- (id)getLiveViewControllerForLiveLogic;
- (id)getLiveOperationView;
- (void)onGetAnchorStatus;
- (void)onGetLiveInfo;
- (void)onLiveClosed:(id)arg1;
- (void)onShareLive:(id)arg1;
- (void)onJoinLive:(id)arg1;
- (void)onCreateLive:(id)arg1;
- (void)switchCameraAction;
- (void)closeRoomAction;
- (id)getLiveCloseDestructiveTitle;
- (void)doHidePauseView;
- (void)doShowPauseViewWithType:(unsigned long long)arg1;
- (id)pauseMessageForType:(unsigned long long)arg1;
- (void)hidePauseViewWithType:(unsigned long long)arg1;
- (void)showPauseViewWithType:(unsigned long long)arg1;
- (void)removeCurrentPauseViewTypes:(unsigned long long)arg1;
- (void)addCurrentPauseViewTypes:(unsigned long long)arg1;
@property(readonly, nonatomic) unsigned long long displayingPauseViewType;
- (_Bool)forbidHiddenNavigationBarWillAppear;
- (unsigned long long)getCurrentSupportedInterfaceOrientations;
- (void)unDisplayContent;
- (void)displayContent;
- (void)minimizeLive:(CDUnknownBlockType)arg1;
- (void)gotoUserProfile:(id)arg1 isAnchorHeadImage:(_Bool)arg2;
- (void)joinLive:(unsigned long long)arg1;
- (void)createLive:(id)arg1;
- (long long)getCurrentOrientation;
- (_Bool)isCurrHasPIPAudioModuleAuthority;
- (void)notifyHideContentView;
- (void)notifyShowContentView;
- (id)pipMinimizeContentView;
- (_Bool)isAnchorPreviewRotateTransiting;
- (void)onEndTransitionWithPreState:(long long)arg1;
- (void)onWillTransitionToMinimize;
- (void)rotateToOrientation:(long long)arg1;
- (_Bool)shouldShowCloseButtonWhenMinimized;
- (_Bool)shouldSupportSpecialLiveAnimatorGestureRecognizer:(id)arg1;
- (_Bool)shouldSupportLiveSwitchingWithGestureRecognizer:(id)arg1;
- (void)onInteractiveTransitionToMinimizationDidBegin;
- (_Bool)isPIPHandlerCreated;
- (void)onLiveMinimizeWindowDidShiftIntoListChanged:(_Bool)arg1;
- (_Bool)shouldInvokeInteractiveTransition;
- (_Bool)shouldSupportInteractiveTransition;
- (void)hideView;
- (void)showView;
- (struct CGRect)getMinimizeSrcFrameForBusiness:(long long)arg1;
@property(readonly, nonatomic) UIView *minimizeSrcView;
@property(readonly, nonatomic) UIView<MMLiveContainerDisplayViewProtocol> *displayContentView;
- (id)createOperationView;
- (void)layoutOperationView;
- (void)layoutBlurView;
- (void)layoutLiveView;
- (void)layoutUI;
- (void)showCameraCastingViewController;
- (void)showLiveStarterViewController;
- (void)initControllerViewSettings;
- (void)initOperationViewHideStatus;
- (void)initUI;
- (void)viewSafeAreaInsetsDidChange;
- (void)unembedChildViewController:(id)arg1;
- (void)embedSubviewFromChildViewController:(id)arg1;
@property(readonly, nonatomic) MMLivePivotViewController *pivotViewController;
@property(readonly, nonatomic) _Bool hasInvokeStartLive;
@property(readonly, nonatomic) MMLiveTask *liveTask;
@property(readonly, nonatomic) long long roleType;
@property(readonly, nonatomic) unsigned long long liveId;
@property(readonly, nonatomic) MMLiveConnectMicLogic *connectMicLogic;
- (void)onLiveAppearingChanged;
- (void)updateLiveProgress;
- (unsigned long long)systemCapabilityAuthScene;
- (void)setupMorphDismiss:(_Bool)arg1;
- (void)persistMorphDismissTarget;
- (id)realNavigationController;
- (id)navigationController;
- (void)showCompleteViewController:(id)arg1;
- (id)createCompleteViewController;
- (void)gotoCompletePage;
- (void)gotoCompletePageOverride;
- (void)closeWithError:(id)arg1 needNotify:(_Bool)arg2;
- (void)closeWithoutNavigation;
- (void)closeByCutLive;
- (void)close;
- (void)anchorAssistantClose;
- (void)stopLive;
- (void)closeDirectlyWithCompletion:(CDUnknownBlockType)arg1;
- (void)closeDirectly;
- (void)changeCurrOrientation:(long long)arg1;
- (void)updateCurrSupportedInterfaceOrientations:(unsigned long long)arg1;
- (_Bool)disableLoadViews;
- (void)markReleaseUIAndLogic;
- (void)exitAndClearLive;
- (long long)preferredStatusBarStyle;
- (_Bool)useBlackStatusbar;
- (_Bool)shouldInteractivePop;
- (unsigned long long)currentOrientationMask;
- (unsigned long long)inner_supportedInterfaceOrientations;
- (unsigned long long)providedInterfaceOrientationMaskInIphone;
- (unsigned long long)supportedInterfaceOrientations;
- (_Bool)shouldAutorotate;
- (void)forceLayoutForOrientationChanged;
- (void)liveViewWillBeAbnormallyDismissed;
- (void)viewDidBeDismissed:(_Bool)arg1;
- (void)viewWillBeDismissed:(_Bool)arg1;
- (void)viewWillTransitionToSize:(struct CGSize)arg1 withTransitionCoordinator:(id)arg2;
- (void)viewDidTransitionToNewSize;
- (void)viewDidLayoutSubviews;
- (void)viewDidDisappear:(_Bool)arg1;
- (void)viewDidAppear:(_Bool)arg1;
- (void)viewWillDisappear:(_Bool)arg1;
- (void)viewWillAppear:(_Bool)arg1;
- (void)unRegisterExtension;
- (void)registerExtension;
- (void)startHandleForLiveRoomDidLoad;
- (void)viewDidLoad;
- (void)initNotifications;
- (void)initReporter;
- (void)initLiveLogic;
- (void)initData;
- (void)initAbnormalState;
- (_Bool)shouldSupportLandScape;
- (_Bool)isJoinLiveAfterAbnormalExit;
- (void)dealloc;
- (id)initWithLiveTaskIdDirectly:(id)arg1;
- (id)initWithLiveTaskId:(id)arg1;
- (id)presentationControllerForPresentedViewController:(id)arg1 presentingViewController:(id)arg2 sourceViewController:(id)arg3;
- (id)animationControllerForPresentedController:(id)arg1 presentingController:(id)arg2 sourceController:(id)arg3;
- (id)animationControllerForDismissedController:(id)arg1;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
// Preceding property had unknown attributes: ?
// Original attribute string: T@"NSString",?,R,C

@property(readonly, copy) NSString *description;
@property(readonly, nonatomic) _Bool enableHorizontalFullscreenInteractiveDismissFromEdge;
// Preceding property had unknown attributes: ?
// Original attribute string: TB,?,R,N

@property(readonly, nonatomic) _Bool enableHorizontalFullscreenInteractiveDismissFromLeftToRight;
// Preceding property had unknown attributes: ?
// Original attribute string: TB,?,R,N

@property(readonly, nonatomic) _Bool enableHorizontalFullscreenMinimization;
// Preceding property had unknown attributes: ?
// Original attribute string: TB,?,R,N

@property(readonly, nonatomic) _Bool enableVerticalFullscreenInteractiveDismissFromBottomToTop;
// Preceding property had unknown attributes: ?
// Original attribute string: TB,?,R,N

@property(readonly, nonatomic) _Bool enableVerticalFullscreenInteractiveDismissFromTopToBottom;
// Preceding property had unknown attributes: ?
// Original attribute string: TB,?,R,N

@property(readonly, nonatomic) _Bool enableVerticalFullscreenMinimization;
// Preceding property had unknown attributes: ?
// Original attribute string: TB,?,R,N

@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;
@property(readonly, nonatomic) _Bool useMorphEgdeGesture;
// Preceding property had unknown attributes: ?
// Original attribute string: TB,?,R,N


@end

