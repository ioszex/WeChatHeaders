//
//     Generated by classdump-c 5.0.1.
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import <objc/NSObject.h>

@class NSMutableArray, TXAudioEffectManager, TXBeautyManager, TXDeviceManager;
@protocol OS_dispatch_queue, TRTCCloudDelegate;

@interface TRTCCloud : NSObject
{
    struct shared_ptr<liteav::trtc::PipelineInterface> _pipeline;
    struct shared_ptr<liteav::trtc::TRTCPipelineClient> _pipelineClient;
    int _screen_source_type;
    struct shared_ptr<liteav::TRTCCloud> _native_cloud_wrapper;
    int _screenStreamType;
    NSObject<OS_dispatch_queue> *_delegateQueue;
    TXDeviceManager *_deviceManager;
    TXBeautyManager *_beautyManager;
    TXAudioEffectManager *_audioEffectManager;
    TRTCCloud *_mainCloud;
    NSMutableArray *_trtcSubClouds;
}

+ (void)setLogDelegate:(id)arg1;
+ (void)setLogDirPath:(id)arg1;
+ (void)setLogCompressEnabled:(_Bool)arg1;
+ (void)setConsoleEnabled:(_Bool)arg1;
+ (void)setLogLevel:(long long)arg1;
+ (id)getSDKVersion;
+ (unsigned long long)generateCustomPTS;
+ (void)destroySharedInstance;
+ (id)sharedInstance;
+ (void)setNetEnv:(int)arg1;
+ (void)initialize;
+ (void)destroySharedIntance;
- (id).cxx_construct;
- (void).cxx_destruct;
@property(retain, nonatomic) NSMutableArray *trtcSubClouds; // @synthesize trtcSubClouds=_trtcSubClouds;
@property(nonatomic) __weak TRTCCloud *mainCloud; // @synthesize mainCloud=_mainCloud;
@property(nonatomic) int screenStreamType; // @synthesize screenStreamType=_screenStreamType;
@property(retain, nonatomic) TXAudioEffectManager *audioEffectManager; // @synthesize audioEffectManager=_audioEffectManager;
@property(retain, nonatomic) TXBeautyManager *beautyManager; // @synthesize beautyManager=_beautyManager;
@property(retain, nonatomic) TXDeviceManager *deviceManager; // @synthesize deviceManager=_deviceManager;
@property(retain, nonatomic) NSObject<OS_dispatch_queue> *delegateQueue; // @synthesize delegateQueue=_delegateQueue;
- (_Bool)isJSONString:(id)arg1 containsKey:(id)arg2 withValue:(id)arg3;
- (int)enablePayloadPrivateEncryption:(_Bool)arg1 params:(id)arg2;
- (id)callExperimentalAPI:(id)arg1;
- (void)setDebugViewMargin:(id)arg1 margin:(struct UIEdgeInsets)arg2;
- (void)showDebugView:(long long)arg1;
- (void)apiLog:(id)arg1;
- (void)stopSpeedTest;
- (int)startSpeedTest:(id)arg1;
- (_Bool)sendSEIMsg:(id)arg1 repeatCount:(int)arg2;
- (_Bool)sendCustomCmdMsg:(long long)arg1 data:(id)arg2 reliable:(_Bool)arg3 ordered:(_Bool)arg4;
- (void)getCustomAudioRenderingFrame:(id)arg1;
- (void)enableCustomAudioRendering:(_Bool)arg1;
- (int)setMixedPlayAudioFrameDelegateFormat:(id)arg1;
- (int)setLocalProcessedAudioFrameDelegateFormat:(id)arg1;
- (int)setCapturedAudioFrameDelegateFormat:(id)arg1;
- (void)setAudioFrameDelegate:(id)arg1;
- (void)setVideoRenderDelegateInternal:(id)arg1 delegate:(id)arg2 pixelFormat:(long long)arg3 bufferType:(long long)arg4;
- (int)setRemoteVideoRenderDelegate:(id)arg1 delegate:(id)arg2 pixelFormat:(long long)arg3 bufferType:(long long)arg4;
- (int)setLocalVideoRenderDelegate:(id)arg1 pixelFormat:(long long)arg2 bufferType:(long long)arg3;
- (int)setLocalVideoProcessDelegete:(id)arg1 pixelFormat:(long long)arg2 bufferType:(long long)arg3;
- (void)setMixExternalAudioVolume:(long long)arg1 playoutVolume:(long long)arg2;
- (int)mixExternalAudioFrame:(id)arg1;
- (void)enableMixExternalAudioFrame:(_Bool)arg1 playout:(_Bool)arg2;
- (void)sendCustomAudioData:(id)arg1;
- (void)enableCustomAudioCapture:(_Bool)arg1;
- (void)sendCustomVideoData:(long long)arg1 frame:(id)arg2;
- (void)enableCustomVideoCapture:(long long)arg1 enable:(_Bool)arg2;
- (void)setSubStreamEncoderParam:(id)arg1;
- (int)resumeScreenCapture;
- (int)pauseScreenCapture;
- (int)stopScreenCapture;
- (void)startScreenCaptureByReplaykit:(long long)arg1 encParam:(id)arg2 appGroup:(id)arg3;
- (void)startScreenCaptureInApp:(long long)arg1 encParam:(id)arg2;
- (void)setSystemAudioLoopbackVolume:(unsigned int)arg1;
- (id)getAudioEffectManager;
- (void)setWatermark:(id)arg1 streamType:(long long)arg2 rect:(struct CGRect)arg3;
- (id)getBeautyManager;
- (id)getDeviceManager;
- (void)set3DSpatialReceivingRange:(id)arg1 range:(long long)arg2;
- (void)updateRemote3DSpatialPosition:(id)arg1 position:(int [3])arg2;
- (void)updateSelf3DSpatialPosition:(int [3])arg1 axisForward:(float [3])arg2 axisRight:(float [3])arg3 axisUp:(float [3])arg4;
- (void)enable3DSpatialAudioEffect:(_Bool)arg1;
- (void)setRemoteAudioParallelParams:(id)arg1;
- (void)stopLocalRecording;
- (void)startLocalRecording:(id)arg1;
- (void)stopAudioRecording;
- (int)startAudioRecording:(id)arg1;
- (void)enableAudioVolumeEvaluation:(_Bool)arg1 withParams:(id)arg2;
- (long long)getAudioPlayoutVolume;
- (void)setAudioPlayoutVolume:(long long)arg1;
- (long long)getAudioCaptureVolume;
- (void)setAudioCaptureVolume:(long long)arg1;
- (void)setRemoteAudioVolume:(id)arg1 volume:(int)arg2;
- (void)setAudioRoute:(long long)arg1;
- (void)muteAllRemoteAudio:(_Bool)arg1;
- (void)muteRemoteAudio:(id)arg1 mute:(_Bool)arg2;
- (void)muteLocalAudio:(_Bool)arg1;
- (void)stopLocalAudio;
- (void)startLocalAudio:(long long)arg1;
- (void)setVideoEncodeParams:(id)arg1 forStream:(int)arg2;
- (void)setGravitySensorAdaptiveMode:(long long)arg1;
- (void)setPerspectiveCorrectionWithUser:(id)arg1 srcPoints:(id)arg2 dstPoints:(id)arg3;
- (void)snapshotVideo:(id)arg1 type:(long long)arg2 sourceType:(unsigned long long)arg3 completionBlock:(CDUnknownBlockType)arg4;
- (void)setRemoteVideoStreamType:(id)arg1 type:(long long)arg2;
- (int)enableEncSmallVideoStream:(_Bool)arg1 withQuality:(id)arg2;
- (void)setRemoteRenderParams:(id)arg1 streamType:(long long)arg2 params:(id)arg3;
- (void)setLocalRenderParams:(id)arg1;
- (void)setNetworkQosParam:(id)arg1;
- (void)setVideoEncoderParam:(id)arg1;
- (void)muteAllRemoteVideoStreams:(_Bool)arg1;
- (void)muteRemoteVideoStream:(id)arg1 streamType:(long long)arg2 mute:(_Bool)arg3;
- (void)stopAllRemoteView;
- (void)stopRemoteView:(id)arg1 streamType:(long long)arg2;
- (void)updateRemoteView:(id)arg1 streamType:(long long)arg2 forUser:(id)arg3;
- (void)startRemoteView:(id)arg1 streamType:(long long)arg2 view:(id)arg3;
- (void)setVideoMuteImage:(id)arg1 fps:(long long)arg2;
- (void)muteLocalVideo:(long long)arg1 mute:(_Bool)arg2;
- (void)stopLocalPreview;
- (void)updateLocalView:(id)arg1;
- (void)startLocalPreview:(_Bool)arg1 view:(id)arg2;
- (void)stopPublishMediaStream:(id)arg1;
- (void)updatePublishMediaStream:(id)arg1 publishTarget:(id)arg2 encoderParam:(id)arg3 mixingConfig:(id)arg4;
- (void)startPublishMediaStream:(id)arg1 encoderParam:(id)arg2 mixingConfig:(id)arg3;
- (void)setDefaultStreamRecvMode:(_Bool)arg1 video:(_Bool)arg2;
- (void)updateOtherRoomForwardMode:(id)arg1;
- (void)disconnectOtherRoom;
- (void)connectOtherRoom:(id)arg1;
- (void)switchRoom:(id)arg1;
- (void)switchRole:(long long)arg1 privateMapKey:(id)arg2;
- (void)switchRole:(long long)arg1;
- (void)exitRoom;
- (void)enterRoom:(id)arg1 appScene:(long long)arg2;
- (void)destroySubCloud:(id)arg1;
- (id)createSubCloud;
- (void)dealloc;
- (id)initSubCloudWithMainCloud:(id)arg1;
- (void)doInit:(_Bool)arg1;
- (id)initMainCloud;
- (id)init;
- (id)getNativeHandle;
- (void)removeDelegate:(id)arg1;
- (void)addDelegate:(id)arg1;
- (struct PipelineInterface *)pipeline;
- (void)setMixTranscodingConfig:(id)arg1;
- (void)stopPublishCDNStream;
- (void)startPublishCDNStream:(id)arg1;
- (void)stopPublishing;
- (void)startPublishing:(id)arg1 type:(long long)arg2;
- (void)setGSensorMode:(long long)arg1;
- (void)setVideoEncoderMirror:(_Bool)arg1;
- (void)setVideoEncoderRotation:(long long)arg1;
- (void)startSpeedTest:(unsigned int)arg1 userId:(id)arg2 userSig:(id)arg3 completion:(CDUnknownBlockType)arg4;
- (void)startRemoteView:(id)arg1 view:(id)arg2;
- (void)startRemoteSubStreamView:(id)arg1 view:(id)arg2;
- (void)muteRemoteVideoStream:(id)arg1 mute:(_Bool)arg2;
- (void)muteLocalVideo:(_Bool)arg1;
- (void)startScreenCaptureByReplaykit:(id)arg1 appGroup:(id)arg2;
- (void)startScreenCaptureInApp:(id)arg1;
- (void)sendCustomVideoData:(id)arg1;
- (void)enableCustomVideoCapture:(_Bool)arg1;
- (void)snapshotVideo:(id)arg1 type:(long long)arg2 completionBlock:(CDUnknownBlockType)arg3;
- (void)setSystemVolumeType:(long long)arg1;
- (_Bool)enbaleTorch:(_Bool)arg1;
- (void)setFocusPosition:(struct CGPoint)arg1;
- (void)setZoom:(double)arg1;
- (void)enableAutoFaceFoucs:(_Bool)arg1;
- (_Bool)isCameraAutoFocusFaceModeSupported;
- (_Bool)isCameraTorchSupported;
- (_Bool)isCameraFocusPositionInPreviewSupported;
- (_Bool)isCameraZoomSupported;
- (void)switchCamera;
- (void)startLocalAudio;
- (void)setAudioQuality:(long long)arg1;
- (void)setPriorRemoteVideoStreamType:(long long)arg1;
- (void)setRemoteSubStreamViewRotation:(id)arg1 rotation:(long long)arg2;
- (void)setRemoteSubStreamViewFillMode:(id)arg1 mode:(long long)arg2;
- (void)stopRemoteSubStreamView:(id)arg1;
- (void)setLocalViewMirror:(unsigned long long)arg1;
- (void)setLocalViewRotation:(long long)arg1;
- (void)setLocalViewFillMode:(long long)arg1;
- (void)setRemoteViewRotation:(id)arg1 rotation:(long long)arg2;
- (void)setRemoteViewFillMode:(id)arg1 mode:(long long)arg2;
- (void)stopRemoteView:(id)arg1;
- (void)enableAudioVolumeEvaluation:(unsigned long long)arg1 enable_vad:(_Bool)arg2;
- (void)enableAudioVolumeEvaluation:(unsigned long long)arg1;
- (void)enableAudioEarMonitoring:(_Bool)arg1;
- (void)resumeAudioEffect:(int)arg1;
- (void)pauseAudioEffect:(int)arg1;
- (void)setAllAudioEffectsVolume:(int)arg1;
- (void)setAudioEffectVolume:(int)arg1 volume:(int)arg2;
- (void)stopAllAudioEffects;
- (void)stopAudioEffect:(int)arg1;
- (void)playAudioEffect:(id)arg1;
- (void)setVoiceChangerType:(long long)arg1;
- (void)setReverbType:(long long)arg1;
- (void)setBGMPublishVolume:(long long)arg1;
- (void)setBGMPlayoutVolume:(long long)arg1;
- (void)setBGMVolume:(long long)arg1;
- (int)setBGMPosition:(long long)arg1;
- (long long)getBGMDuration:(id)arg1;
- (void)resumeBGM;
- (void)pauseBGM;
- (void)stopBGM;
- (void)playBGM:(id)arg1 withBeginNotify:(CDUnknownBlockType)arg2 withProgressNotify:(CDUnknownBlockType)arg3 andCompleteNotify:(CDUnknownBlockType)arg4;
- (void)setFilterConcentration:(float)arg1;
- (void)setFilter:(id)arg1;
- (void)setMotionMute:(_Bool)arg1;
- (void)selectMotionTmpl:(id)arg1;
- (void)setNoseSlimLevel:(float)arg1;
- (void)setFaceShortLevel:(float)arg1;
- (void)setChinLevel:(float)arg1;
- (void)setFaceVLevel:(float)arg1;
- (void)setFaceScaleLevel:(float)arg1;
- (void)setEyeScaleLevel:(float)arg1;
- (void)setGreenScreenFile:(id)arg1;
- (void)setBeautyStyle:(long long)arg1 beautyLevel:(long long)arg2 whitenessLevel:(long long)arg3 ruddinessLevel:(long long)arg4;
- (void)setMicVolumeOnMixing:(long long)arg1;
@property(nonatomic) __weak id <TRTCCloudDelegate> delegate;

@end

