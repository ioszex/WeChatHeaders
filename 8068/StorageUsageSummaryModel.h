//
//     Generated by classdump-c 5.0.1.
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import <objc/NSObject.h>

@class NSMutableArray;

@interface StorageUsageSummaryModel : NSObject
{
    _Bool _useCompactLayout;
    unsigned long long _appSize;
    NSMutableArray *_detailModels;
    long long _deviceSize;
    long long _freeSize;
}

- (void).cxx_destruct;
@property(nonatomic) _Bool useCompactLayout; // @synthesize useCompactLayout=_useCompactLayout;
@property(nonatomic) long long freeSize; // @synthesize freeSize=_freeSize;
@property(nonatomic) long long deviceSize; // @synthesize deviceSize=_deviceSize;
@property(retain, nonatomic) NSMutableArray *detailModels; // @synthesize detailModels=_detailModels;
@property(nonatomic) unsigned long long appSize; // @synthesize appSize=_appSize;
- (_Bool)p_isEmptyChatLog;
- (id)description;
- (id)getCacheDes;
- (void)upadateRawSizeToShow;
- (void)updateAppSizeWithDetailSize;
- (void)initForLoading;
- (void)refreshModelWithAppGroup:(unsigned long long)arg1 canCleanCache:(unsigned long long)arg2 otherUser:(unsigned long long)arg3 vacuumChatLog:(unsigned long long)arg4 vacuumOther:(unsigned long long)arg5 deepCleanResource:(unsigned long long)arg6 isEmptySessionZeroSize:(_Bool)arg7;
- (id)initWithScanStat:(id)arg1 useCompactLayout:(_Bool)arg2;
- (id)initWithUseCompactLayout:(_Bool)arg1;
- (id)init;

@end

