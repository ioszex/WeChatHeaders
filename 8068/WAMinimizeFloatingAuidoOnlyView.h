//
//     Generated by classdump-c 5.0.1.
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import <UIKit/UIView.h>

@class MMUILabel, UIFont, WAMinimizeFloatingAudioSoundView;

@interface WAMinimizeFloatingAuidoOnlyView : UIView
{
    _Bool _soundActive;
    double _sideViewMarginCoefficient;
    MMUILabel *_indicatorLabel;
    WAMinimizeFloatingAudioSoundView *_leftView;
    WAMinimizeFloatingAudioSoundView *_rightView;
}

- (void).cxx_destruct;
@property(retain, nonatomic) WAMinimizeFloatingAudioSoundView *rightView; // @synthesize rightView=_rightView;
@property(retain, nonatomic) WAMinimizeFloatingAudioSoundView *leftView; // @synthesize leftView=_leftView;
@property(retain, nonatomic) MMUILabel *indicatorLabel; // @synthesize indicatorLabel=_indicatorLabel;
@property(nonatomic) double sideViewMarginCoefficient; // @synthesize sideViewMarginCoefficient=_sideViewMarginCoefficient;
@property(nonatomic) _Bool soundActive; // @synthesize soundActive=_soundActive;
- (void)updateSoundActive:(_Bool)arg1;
- (void)layoutSubviews;
- (struct CGSize)sizeThatFits:(struct CGSize)arg1;
- (id)displayText;
@property(retain, nonatomic) UIFont *labelFont;
- (id)initWithFrame:(struct CGRect)arg1 isStub:(_Bool)arg2;

@end

